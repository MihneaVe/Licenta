...
export default function Topics() {
  ...
  async function fetchData() {
    ...
    const [{ data: agg }, { data: sample }] = await Promise.all([
      ...
      supabase.from('feedbacks').select('*').order('created_at', { ascending: false }).limit(500),
    ]);

    if (!agg) { setLoading(false); return; }

    const byTopic = {};
    (sample || []).forEach((f) => { (byTopic[f.topic] ||= []).push(f); });

    const total = agg.reduce((s, t) => s + (t.mentions || 0), 0);
    const aggregated = agg
      .map((t) => ({
        name: t.name,
        count: t.mentions,
        avgScore: t.avg_score,
        pct: total ? Math.round((t.mentions / total) * 100) : 0,
        feedbacks: byTopic[t.name] || [],
      }))
  ...
  // sentiment_score is 0-100 (50 = neutral, higher = more positive).
  const sentimentTrend = (score) => {
    if (score >= 60) return { icon: 'trending_up', cls: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30' };
    if (score >= 40) return { icon: 'trending_flat', cls: 'text-amber-600 bg-amber-50 dark:bg-amber-900/30' };
    return { icon: 'trending_down', cls: 'text-rose-600 bg-rose-50 dark:bg-rose-900/30' };
  };
