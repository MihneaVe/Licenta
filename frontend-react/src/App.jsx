...
const AuthContainer = ({ children }) => {
  ...
  // Local preview: skip the Auth0 session gate entirely (VITE_PREVIEW_NOAUTH=1)
  // so the dashboard renders immediately without waiting on the login flow.
  if (import.meta.env.VITE_PREVIEW_NOAUTH === '1') {
    return children;
  }
...
const ProtectedRoute = ({ children }) => {
  ...
  // Local preview escape hatch: with VITE_PREVIEW_NOAUTH=1 the dashboard
  // renders without an Auth0 login (used to view data locally). Has no
  // effect unless that env var is set, so production behaviour is unchanged.
  const previewNoAuth = import.meta.env.VITE_PREVIEW_NOAUTH === '1';

  // If not authenticated, bounce them back to the landing page
  if (!isAuthenticated && !previewNoAuth) {
